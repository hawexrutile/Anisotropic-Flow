{  //Code for plotting d(N)/d(N\y) vs y;  rapidity is denoted by y
    lister("ampt.dat",111);
    cout<<listid.size();
    TH1D *h1= new TH1D("h1","rapidity distribution",50,-10,10);
    ...
    for (int j=0 ;j<50;j++){            //*Taking counts from each bin, subtracting adjacent bin heights and dividing by bin width
        
        double l= h1->GetBinContent(j);
        double u= h1->GetBinContent(j+1);
//        listdndy[j]=(((5-(-5)/40))/(u-l));
        // if (u!=l){
         listdndy[j]=(u-l)/((50.0/(5.0-(-5.0))));
         listycent[j]=(h1->GetBinCenter(j));
        //  cout<<listdndy[j]<<"      "<<listycent[j]<<"\t"<< j<<"\n";
        // };
        
    };
    TGraph *g2= new TGraph(50,listycent,listdndy);
