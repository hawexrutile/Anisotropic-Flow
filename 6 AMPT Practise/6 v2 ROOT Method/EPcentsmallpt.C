...
TFile *f=new TFile("CentralityDist.root");//includes TH2F *myHist2;                                                                          
	...
	for (int c=0;c<N;c++){
			if (multi>lowcentral[c] && multi <highcentral[c]){
				...
				for(int i=0; i<nTrack; i++){
					...
					if (denopos==0||denoneg==0) continue;      
					...
					hpRes[c]->Fill(1,TMath::Cos(2*Psi2pos-2*Psi2neg));
